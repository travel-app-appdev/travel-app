...
and in controllers folder is tripsController.ts:
...
Right — then you already have the GET trips flow implemented. The next step is just to extend the same file with trip creation.

    What to add
In routes/trips.ts, add a POST route next to getMyTrips:
...
export default router;
That is the standard Express pattern: same route module, multiple handlers for the same feature area.

    What to add in tripsController.ts
You can keep getMyTrips exactly as it is and add this below it:
