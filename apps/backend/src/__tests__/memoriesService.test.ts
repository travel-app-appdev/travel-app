...
describe("memoriesService", () => {
  beforeEach(() => {
    ...
    (findMemoryPhotoById as jest.Mock).mockResolvedValue({
      memory_id: "memory-123",
      trip_id: TRIP_ID,
      file_name: "memory-123.png",
    });
  });
  ...
  it("deletes an existing memory photo and uploaded file", async () => {
    ...
    expect(deleteMemoryPhotoById).toHaveBeenCalledWith("memory-123");
    expect(fs.rm).toHaveBeenCalledWith(
      expect.stringContaining("memory-123.png"),
      { force: true },
    );
  });
});
